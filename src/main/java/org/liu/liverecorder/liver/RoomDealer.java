package org.liu.liverecorder.liver;

import org.liu.liverecorder.liver.domain.RoomInfo;
import org.liu.liverecorder.util.HttpRequestUtil;

public abstract class RoomDealer {
    public HttpRequestUtil util = new HttpRequestUtil();
    protected nicelee.bilibili.util.HttpHeaders headers = new nicelee.bilibili.util.HttpHeaders();
    public Integer currentIndex = 0; // 当前任务编号
    protected String cookie;

    public static RoomDealer createRoomDealer(String liver) {
        Class<?> clazz = nicelee.bilibili.PackageScanLoader.dealerClazz.get(liver);
        try {
